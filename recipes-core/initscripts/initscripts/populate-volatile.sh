...
# The built-in populate-volatile.sh script trys to "rm -rf /tmp" in the link_file() function, which gives a warning during Linux bootup.
# rm: can't remove "///tmp" device or resource busy
# Get rid of this warning by .bbappend the initscript_1.0.bb file
...
link_file() {
	EXEC="
	if [ -L \"$2\" ]; then
		[ \"\$(readlink -f \"$2\")\" != \"\$(readlink -f \"$1\")\" ] && { ln -sf \"$1\" \"$2\"; };
	elif [ -d \"$2\" ]; then
